...
model = keras.models.load_model('Model/ResNet50_epochs50_false_unfreeze10.h5')
columns = ["Image N", "Ble dur", "Avoine", "Orge", "Triticale", "Ble tendre", "Total"]
...
for i, file in enumerate(files):
    ...
    sum_prediction = [i+1, y_pred.count(1), y_pred.count(0), y_pred.count(3), y_pred.count(4), y_pred.count(2),
                      y_pred.count(0) + y_pred.count(1) + y_pred.count(2) + y_pred.count(3) + y_pred.count(4)]

    df.loc[len(df)] = sum_prediction
df.to_csv('Results CNCC.csv', index=False)
