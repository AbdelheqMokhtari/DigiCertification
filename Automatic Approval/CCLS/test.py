...
classes = ['Num of image', 'ble dur', 'Ble tendre', 'casee', 'echaudes', 'maigre', 'metadine', 'Mouchten', 'piqee']
Names = pd.DataFrame(columns=classes)

df = pd.DataFrame(columns=columns)

image = cv2.imread("Image/Bousselam 6.jpg")
...
loaded_model = joblib.load('Model/Features CNCC V1 Normalized.pkl')

# Use the loaded model to make predictions on the features
y_pred = loaded_model.predict(normalized_df)

print(y_pred)

y_pred = y_pred.tolist()

sum_prediction = [1, y_pred.count(0), y_pred.count(1), y_pred.count(2), y_pred.count(3), y_pred.count(4),
                  y_pred.count(5), y_pred.count(6), y_pred.count(7)]

Names.loc[len(Names)] = sum_prediction
Names.to_csv('Results CCLS.csv', index=False)