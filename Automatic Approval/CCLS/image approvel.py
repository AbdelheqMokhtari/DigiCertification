...
image = cv.imread("Image/Winter Wheat 2.jpg")
...
gray = cv.cvtColor(gamma_img, cv.COLOR_BGR2GRAY)

# Define the structuring element
kernel = np.ones((21, 21), np.uint8)

# Perform close operation
img_close = cv.morphologyEx(gray, cv.MORPH_CLOSE, kernel)

# Apply median blur
img_blur = cv.medianBlur(img_close, 15)  # Here, 15 is the kernel size

# Apply Otsu's method to automatically determine the threshold value
thresh_val, thresh_img = cv.threshold(img_blur, 0, 255, cv.THRESH_BINARY + cv.THRESH_OTSU)
...
for contour in contours:
    if cv.contourArea(contour) > 1000:  # Minimum area threshold
        cv.drawContours(image, [contour], 0, (0, 255, 0), 5)
        total_number += 1
...
for contour in contours:
    if cv.contourArea(contour) > 1000:  # Minimum area threshold
        ...
        print("Area =", cv.contourArea(contour), "x =", x, "y =", y, "w =", w, "h =", h)
        ...
        output_img = cv.copyMakeBorder(crop_img, h_pad, h_pad, w_pad, w_pad, cv.BORDER_CONSTANT, value=(0, 0, 0))

        # crop image
        crop_images.append(output_img)
